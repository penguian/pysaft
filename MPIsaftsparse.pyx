...
def build_dna_frequency_lists(file_name, unsigned int word_len, mpi_rank=0, mpi_size=1, desc=True, masked=True):
...
def build_dna_sparse_frequency_matrix(file_name, word_len, mpi_rank=0, mpi_size=1, desc=True, masked=True):
    freq_list, size_list, desc_list = build_dna_frequency_lists(file_name, word_len, mpi_rank, mpi_size, desc, masked)
